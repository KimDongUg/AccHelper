...
async function loadCompanies() {
    ...
    try {
        ...
        // "+ 회사 등록" 카드를 그리드 맨 앞에 추가
        const regCard = document.createElement('a');
        regCard.href = '/company-register.html';
        regCard.className = 'company-card company-card-register';
        regCard.innerHTML =
            '<div class="company-card-icon company-card-icon-register">' +
            ...
            '</div>' +
            '<div class="company-card-info"><h3>회사 등록</h3></div>';
        companyGrid.insertBefore(regCard, companyGrid.firstChild);
    } catch (err) {
        companyLoading.style.display = 'none';
